...
class JZBLECentralManager: NSObject {
    ...
    //只读的设备过滤关键字
    var scanningFilterString : String {
        get {
            return (self._peripheralNameContaningString ?? "")
        }
    }
...
extension JZBLECentralManager {
    ...
    ///停止扫描
    func stopScanningPeripherals() -> JZBLECentralManager {
        //判断是否有启用倒计时
    ...
    ///连接到指定设备
    func connectToPeripheral(peripheral : CBPeripheral) -> JZBLECentralManager {
...
//        self.removePeripheralInConnectedList(peripheral)
//        
//        if !self._foundPeripherals.contains(peripheral) {
...
//        }
    ...
    ///清除所有已发现的设备
    func cleanAllPeripherals() -> JZBLECentralManager {
        self._foundPeripherals.removeAll()
        
        return self
    }
    ///从发现列表移除指定设备
    private func removePeripheralInFoundList(peripheral : CBPeripheral) {
        if self._foundPeripherals.contains(peripheral) {
            for (index, tempPeripheral) in self._foundPeripherals.enumerate() {
                if tempPeripheral == peripheral {
                    self._foundPeripherals.removeAtIndex(index)
                }
            }
        }
    ...
    ///从连接列表移除指定设备
    private func removePeripheralInConnectedList(peripheral : CBPeripheral) {
...
extension JZBLECentralManager : CBCentralManagerDelegate {
    ...
    @objc internal func centralManager(central: CBCentralManager, didDiscoverPeripheral peripheral: CBPeripheral, advertisementData: [String : AnyObject], RSSI: NSNumber) {
        ...
        if !self.foundPeripherals.contains(peripheral) {
            ...
            } else if self._isToConnectPeripheral {
                //没有设置回调闭包，判断是否设置自动连接
                isToConnect = true
            }
    ...
    @objc internal func centralManager(central: CBCentralManager, didConnectPeripheral peripheral: CBPeripheral) {
        ...
        //查看连接成功的设备是否在发现列表中
        self.removePeripheralInFoundList(peripheral)
        //设置外围设备的代理为当前视图控制器
