...
extension JZBLEPeripheralManager {
    ...
    ///用随机UUID生成服务
    func generateRandomService() -> CBMutableService {
        return CBMutableService(type: self.generateRandomUUID(), primary: true)
    }
    ///用随机UUID生成可读特征
    func generateRandomReadableCharacteristic(value : NSData? = nil) -> CBMutableCharacteristic {
        return CBMutableCharacteristic(type: self.generateRandomUUID(), properties: .Read, value: value, permissions: .Readable)
    }
    ///用随机UUID生成可写特征
    func generateRandomWriteableCharacteristic(value : NSData? = nil) -> CBMutableCharacteristic {
        return CBMutableCharacteristic(type: self.generateRandomUUID(), properties: .Write, value: value, permissions: .Writeable)
    }
    ///用随机UUID生成通知特征
    func generateRandomNotifyCharacteristic(value : NSData? = nil) -> CBMutableCharacteristic {
        return CBMutableCharacteristic(type: self.generateRandomUUID(), properties: .Notify, value: value, permissions: .Readable)
    }
    ///用随机UUID生成描述
    func generateRandomDescriptor(value : AnyObject? = nil) -> CBMutableDescriptor {
        return CBMutableDescriptor(type: self.generateRandomUUID(), value: value)
    }
    ///用指定UUID生成服务
    func generateServiceWithUUID(UUID : CBUUID) -> CBMutableService {
        return CBMutableService(type: UUID, primary: true)
    }
    ///用指定UUID生成可读特征
    func generateReadableCharacteristicWithUUID(UUID : CBUUID, value : NSData? = nil) -> CBMutableCharacteristic {
        return CBMutableCharacteristic(type: UUID, properties: .Read, value: value, permissions: .Readable)
    }
    ///用指定UUID生成可写特征
    func generateWriteableCharacteristicWithUUID(UUID : CBUUID, value : NSData? = nil) -> CBMutableCharacteristic {
        return CBMutableCharacteristic(type: UUID, properties: .Write, value: value, permissions: .Writeable)
    }
    ///用指定UUID生成通知特征
    func generateNotifyCharacteristicWithUUID(UUID : CBUUID, value : NSData? = nil) -> CBMutableCharacteristic {
        return CBMutableCharacteristic(type: UUID, properties: .Notify, value: value, permissions: .Readable)
    }
    ///用指定UUID生成描述
    func generateDescriptorWithUUID(UUID : CBUUID, value : AnyObject? = nil) -> CBMutableDescriptor {
