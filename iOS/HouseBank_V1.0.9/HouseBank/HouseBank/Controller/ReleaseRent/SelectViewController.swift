...
//                        first = pinyin.substringToIndex(advancedBy(pinyin.startIndex, 1))
                        first = pinyin.substringToIndex(pinyin.startIndex)
                    }
                    
                    if let arrayInData : NSMutableArray = self._dataDicts[first]{
                        arrayInData.addObject(dict)
                    }else{
                        let arrayInData = NSMutableArray()
                        arrayInData.addObject(dict)
